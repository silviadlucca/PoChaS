...
<main>
  ...
  <section>
    ...
    <table>
      <thead><tr><th>Ancla</th><th>Distancia UWB</th><th>RSSI</th><th>PD</th></tr></thead>
...
function render(state) {
  ...
  document.getElementById("readings").innerHTML = readings.length
    ? readings.map(r => `<tr><td class="mono">${r.id}</td><td>${fmt(r.distance)} m</td><td>${fmt(r.rssi, 1)} dBm</td><td>${fmt(r.power_diff, 2)} dB</td></tr>`).join("")
