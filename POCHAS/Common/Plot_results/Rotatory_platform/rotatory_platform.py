...
def cargar_datos(nombre_archivo):
    ...
    try:
        with open(nombre_archivo, 'r') as f:
            for linea in f:
                linea = linea.strip()
                if not linea or linea.startswith('#') or linea.startswith('Latitude'):
                    continue
                partes = linea.split(',')
                if len(partes) >= 2:
...
# --- CONFIGURACIÓN Y CARGA AUTOMÁTICA ---

# 1. Buscar todos los archivos .txt en la carpeta actual
# (Si tus archivos empiezan por 'Measure_', puedes usar 'Measure_*.txt' para ser más preciso)
archivos_txt = glob.glob("*.txt")

if not archivos_txt:
    print("Error: No se ha encontrado ningún archivo .txt en la carpeta.")
else:
    # 2. Seleccionar el archivo más reciente (el último modificado/creado)
    archivo = max(archivos_txt, key=os.path.getctime)
    print(f"Cargando automáticamente el último archivo: {archivo}")
    ...
    if rssi:
        ...
        if max(rssi) != min(rssi): # Protección extra
